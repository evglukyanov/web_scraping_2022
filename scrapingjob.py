...
class ScrapingJob():
    ...
    def _parser_item_superjob(self, item):
        ...
        # city
        company_location = item.find('span', {'class': 'f-test-text-company-item-location'}).findChildren()[
            1].getText().split(',')
        ...
        if salary:
            ...
            is_check_sarary = \
            item.find('span', {'class': 'f-test-text-company-item-salary'}).getText().replace(u'\xa0', u' ').split(' ',
                                                                                                                   1)[0]
    ...
    def _get_last_page_superjob(self, html):
        ...
        if parsed_html:
            ...
            if not page_block:
                ...
                last_page = int(
                    page_block.find_all('a')[-2].getText()
                )
